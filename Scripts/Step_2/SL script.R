...
#transfer
data<-readMat("../Step_1/data_set.mat")

#save
save(data,file="dataapplication.RData")
...
#data$D.p.d [1:52, 1:2902]
#data$dBE.eV.d [1:52, 1]
#data$D.p.A [1:52, 1:2017]
#data$dBE.eV.A [1:52, 1]
...
write.table(lsname, "./MgO_dopant_LS.txt", sep="\t", row.names = F, col.names = F)
lsname
...
write.table(namexmat[which(hsbeta1!=0)], "./MgO_dopant_HS.txt", sep="\t", row.names = F, col.names = F)
...
write.table(namexmat[which(dlbeta1!=0)], "./MgO_dopant_DL.txt", sep="\t", row.names = F, col.names = F)
...
#DL normalize
dlbeta=countnormalize(dlresult)
