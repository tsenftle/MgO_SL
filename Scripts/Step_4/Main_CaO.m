...
[~, ~, raw] = xlsread('BindingEnergy_CaO_V1.xlsx','binding_energy_difference');
raw = raw(2:53,:);
cellVectors = raw(:,[1,2]);
raw = raw(:,3);
...
[~, ~, raw] = xlsread('BindingEnergy_CaO_V1.xlsx','atomic_properties');
raw = raw(3:17,:);
cellVectors = raw(:,1);
raw = raw(1:15,2:17);
data = reshape([raw{:}], size(raw));
...
[~, ~, raw] = xlsread('BindingEnergy_CaO_V1.xlsx','dopant_properties');
raw = raw(3:end,:);
...
for i=1:n_s
    ...
    ids=find(strcmp(atom_name, 'Ca'));               % index of parent metal in oxide surface
...
% %% Remove descriptors that variance is zero
% Unit_temp = ones(1, size(D_p_d,2));
% [D_p_d_2, head_d_2, Unit_temp_2] = RmVarZero(D_p_d, head_d, Unit_temp, 10^-10);
% [D_p_d_3, head_d_3, Unit_temp_3] = RmIdentical(D_p_d_2, head_d_2, Unit_temp_2);

save('data_set_CaO.mat')
