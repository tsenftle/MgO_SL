...
[~, ~, raw] = xlsread('BindingEnergy_dopant_ZnO_V1.xlsx','binding_energy_difference');
raw = raw(2:53,:);
cellVectors = raw(:,[1,2]);
raw = raw(:,3);
...
[~, ~, raw] = xlsread('BindingEnergy_dopant_ZnO_V1.xlsx','atomic_properties');
raw = raw(3:17,:);
cellVectors = raw(:,1);
raw = raw(1:15,2:17);
data = reshape([raw{:}], size(raw));
...
[~, ~, raw] = xlsread('BindingEnergy_dopant_ZnO_V1.xlsx','dopant_properties');
raw = raw(3:6,:);
...
for i=1:n_s
    ...
    ids=find(strcmp(atom_name, 'Zn'));               % index of parent metal in oxide surface
