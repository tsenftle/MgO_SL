...
[~, ~, raw] = xlsread('BindingEnergyV1.xlsx','binding_energy_difference');
raw = raw(2:105,:);
cellVectors = raw(:,[1,2]);
raw = raw(:,3);
...
dBE_eV      =   data;                  % Binding energy difference between ...
...
[~, ~, raw] = xlsread('BindingEnergyV1.xlsx','atomic_properties');
raw = raw(3:17,:);
cellVectors = raw(:,1);
raw = raw(1:15,2:17);
data = reshape([raw{:}], size(raw));
...
[~, ~, raw] = xlsread('BindingEnergyV1.xlsx','dopant_properties');
raw = raw(3:end,:);
...
[~, ~, raw] = xlsread('BindingEnergyV1.xlsx','adsorbate_properties');
raw = raw(3:end,:);
cellVectors = raw(:,1);
raw = raw(1:4,2:11);
data = reshape([raw{:}], size(raw));
...
A_PA_eV = data(:,4)/60.2/1.602;      % Proton affinity (eV)
A_cn = data(:,5);                    % Coordination number of adsorbate on surface
A_DE_eV = data(:,6)/60.2/1.602;      % Bond disscociation energy (eV)
A_EN_P = data(:,7);                  % Electronegativity - Pauling scale of attached atom
A_EN_P_del = data(:,8);              % Electronegativity - Pauling scale - difference between adsorbate atoms
A_EN_MB = data(:,9);                 % Electronegativity - Martynov-Batsanov of attached atom
A_EN_MB_del = data(:,10);            % Electronegativity - Martynov-Batsanov - difference between adsorbate atoms
...
n_s  = size(dBE_eV,1);          % Number of samples
n_ds = 52;                      % Number of samples related to doped surface
...
head_A = {'a_IE_eV', 'a_EA_eV', 'a_Val_elec', 'a_PA_eV', 'a_cn', 'a_DE_eV',...
...
D_p_d = D_p_d(n_s-n_ds+1:end,:);
dBE_eV_d = dBE_eV(n_s-n_ds+1:end);

% Adsorbate properties
for i=1:n_s-n_ds
    ida=find(strcmp(adsorbate_name, AdsDop(i)));     % index of adsorbate
    D_p_A(i,:)= [ A_IE_eV(ida) A_EA_eV(ida) A_Val_elec(ida) A_PA_eV(ida) A_cn(ida)...
                  A_DE_eV(ida) A_EN_P(ida) A_EN_P_del(ida) A_EN_MB(ida) A_EN_MB_del(ida)];
end

D_p_A = D_p_A(1:n_ds,:);
dBE_eV_A = dBE_eV(1:n_ds);
...
%% Choose only matrix for electronegativity, ionization energy and electron affinity for dopant

D_p_d_Elec = [ D_p_a D_p_d D_p_d_EN_P D_p_d_EN_MB D_p_d_IE D_p_d_IE_N D_p_d_EA D_p_d_EA_N D_p_d_AEN];
head_d_Elec = [ head_a head_d head_d_EN_P head_d_EN_MB head_d_IE head_d_IE_N head_d_EA head_d_EA_N head_d_AEN];
...
Data_temp = [D_p_a(:,4:6) D_p_A(:,7:8)];
Head_temp = [head_a(4:6) head_A(7:8)];
Unit_temp = ones(1, size(Data_temp,2));
Operation_1 = {'-abs'};
[Data_temp_1, Head_temp_1, Unit_temp_1] = GenBasicFeature(Data_temp, Head_temp, Unit_temp, Operation_1, 1);
Operation_2 = {'/abs'};
...
Data_temp = [D_p_a(:,7:9) D_p_A(:,9:10)];
Head_temp = [head_a(7:9) head_A(9:10)];
Unit_temp = ones(1, size(Data_temp,2));
Operation_1 = {'-abs'};
[Data_temp_1, Head_temp_1, Unit_temp_1] = GenBasicFeature(Data_temp, Head_temp, Unit_temp, Operation_1, 1);
Operation_2 = {'/abs'};
